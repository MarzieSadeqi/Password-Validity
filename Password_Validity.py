...
def check_password(password):
    ...
    # these items must be check to track required character types
    has_lower = False
    has_upper = False
    digit_count = 0
    has_special = False
    ...
    for char in password:
        if 'a' <= char <= 'z':  # Lowercase letter found
            has_lower = True
        elif 'A' <= char <= 'Z':  # Uppercase letter found
            has_upper = True
        elif '0' <= char <= '9':  # Digit found
            digit_count += 1
        elif char in special_chars:  # Special character found
            has_special = True
